...
with open("back_translation_tir_2.txt", "r", encoding="utf-8") as f:
    back_translation_tir_2_lines = [line.strip() for line in f if line.strip()]

assert len(tir_zero_shot) == len(back_translation_tir_2_lines), "The number of lines in the document is inconsistent！"
...
for idx, (orig, back) in enumerate(tqdm(zip(tir_zero_shot, back_translation_tir_2_lines), total=len(tir_zero_shot))):
    prompt = (
...
for r in results:
    ...
    print(f"Backtrn:  {r['back_translation_tir_2']}")
